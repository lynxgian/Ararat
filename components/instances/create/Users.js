...
export default function Users({ setPage, setUsers, users }) {
  ...
  return (
    <Dialog open={true}>
      <DialogTitle>
        <Typography variant="h6" fontFamily="Poppins" align="center">
          Users
        </Typography>
      </DialogTitle>
              ...
              <>
                ...
                <Grid container>
                  <FormControlLabel
                    control={
                      <Checkbox
                        checked={tempUser.permissions.all}
                        onChange={(e) => {
                          let perms = [];
                          if (e.target.checked) {
                            Object.keys(permissions.instance).forEach((key) => {
                              Object.keys(permissions.instance[key]).forEach(
                                (key2) => {
                                  permissions.instance[key][key2].forEach(
                                    (permission) => {
                ...
                {Object.keys(permissions.instance).map((key) => {
                  return (
                    <Accordion
                      key={key}
                      sx={{ mt: 1 }}
                      expanded={userPanel == key}
                      onChange={() =>
                        userPanel == key ? setUserPanel('') : setUserPanel(key)
                      }
                    >
                      ...
                      <AccordionDetails>
                        <Grid container direction="column">
                          {Object.keys(permissions.instance[key]).map(
                            (key2) => {
                              return (
                                <>
                                  <Typography fontWeight={600}>
                                    {key2[0].toUpperCase() +
                                      key2.slice(1, key2.length)}
                                  </Typography>
                                  <Grid container>
                                    {permissions.instance[key][key2].map(
                                      (yes) => {
                                        return (
                                          <>
                                            <FormControlLabel
                                              control={
                                                <Checkbox
                                                  checked={tempUser.permissions.includes(
                                                    yes
                                                  )}
                                                  onChange={(e) => {
                                                    let perms =
            ...
            >
              <Grid container direction="row" sx={{ mt: 1, mb: 1 }}>
                ...
                <Grid
                  container
                  direction="row"
                  sx={{ mr: 'auto', ml: 5, mt: 2, mb: 2, maxWidth: '55%' }}
                >
                    ...
                    <>
                      ...
                      >
                        {(() => {
                          ...
                          const checkPermissions = (perms, userPerms) => {
                            return Object.entries(perms).reduce(
                          ...
                          const checkedPermissions = checkPermissions(
                            formattedPermissions,
                            user.permissions
                          );
                          console.info(formattedPermissions)
                          return checkedPermissions.map((perm) => {
                            return (
                              <>
